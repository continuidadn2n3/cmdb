    ...
    initSpinner: function () {
        ...
        navLinks.forEach(link => {
            link.addEventListener('click', function (event) {
                // Si es un botón de submit dentro de un form, validamos primero
                if (link.tagName === 'BUTTON' && link.type === 'submit' && link.form) {
                    if (!link.form.checkValidity()) {
                        // El form es inválido, dejamos que el navegador muestre el error
                        // y NO mostramos el spinner.
                        return;
                    }
                }
                spinner.style.display = 'flex';
